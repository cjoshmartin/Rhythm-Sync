		...
		function(err, user) {
			...
			oauth.get(
				...
				function (err, data, res) {
					...
					request(
					    { method: 'POST'
					    , uri: 'https://qh.azure-api.net/dev/FitbitFlex?subscription-key=3beb82658857434d8dad843a076df43d'
                        , body: JSON.stringify({ 
                                step: data.summary.steps, 
                                dist: data.summary.distances[0].distance, 
                                acal: data.summary.activityCalories,
...
function callUserCallback(err, user) {
	if (err) {
...
function notificationsReceived(req, res) {
	...
	fs.readFile(req.files.updates.path, {encoding: 'utf8'}, function (err, data) {
		...
		for (var i = 0; i < data.length; i++) {
			console.log(data[i]);
			updateUserSteps(data[i].ownerId, callUserCallback);
		}
	});
};
